...
main(int argc, char *argv[])
{
	...
	server_host=gethostbyname(argv[1]); 					//uses ips
	...
	server_port=atoi(argv[2]); 								//gets port
	strcpy(client_send_string,argv[3]); 					//gets string
	...
	out_cnt=sendto(csd, client_send_string, string_size, 0,      //send the string via sendto
